...
export async function triggerAutoSummaries(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.summary.trigger_auto_summaries', async (span) => {
    try {
      ...
      if (result.failed.length > 0) {
        ...
        span.setAttribute('commit_story.summary.failed_count', result.failed.length);
        return result;
      }
      ...
      span.setAttribute('commit_story.summary.failed_count', result.failed.length + weeklyResult.failed.length + monthlyResult.failed.length);
...
export async function triggerAutoWeeklySummaries(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.summary.trigger_auto_weekly_summaries', async (span) => {
    try {
      const { onProgress } = options;

      const unsummarizedWeeks = await findUnsummarizedWeeks(basePath);
      span.setAttribute('commit_story.summary.unsummarized_weeks_count', unsummarizedWeeks.length);
...
export async function triggerAutoMonthlySummaries(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.summary.trigger_auto_monthly_summaries', async (span) => {
    try {
      const { onProgress } = options;

      const unsummarizedMonths = await findUnsummarizedMonths(basePath);
      span.setAttribute('commit_story.summary.unsummarized_months_count', unsummarizedMonths.length);
