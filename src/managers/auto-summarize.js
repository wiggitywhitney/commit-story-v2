...
export async function triggerAutoSummaries(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.managers.trigger_auto_summaries', async (span) => {
    try {
      span.setAttribute('commit_story.journal.base_path', basePath);
      ...
      span.setAttribute('commit_story.summarize.failed_count', result.failed.length + weeklyResult.failed.length + monthlyResult.failed.length);
      return {
...
export async function triggerAutoWeeklySummaries(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.managers.trigger_auto_weekly_summaries', async (span) => {
    try {
      span.setAttribute('commit_story.journal.base_path', basePath);
      const { onProgress } = options;
...
export async function triggerAutoMonthlySummaries(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.managers.trigger_auto_monthly_summaries', async (span) => {
    try {
      span.setAttribute('commit_story.journal.base_path', basePath);
      const { onProgress } = options;
