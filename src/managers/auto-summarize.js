...
export async function triggerAutoSummaries(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.commands.trigger_auto_summaries', async (span) => {
    try {
      const { onProgress } = options;

      const unsummarizedDays = await findUnsummarizedDays(basePath);
      span.setAttribute('commit_story.journal.unsummarized_days_count', unsummarizedDays.length);
      ...
      if (result.failed.length > 0) {
        ...
        span.setAttribute('commit_story.journal.summaries_count', result.generated.length);
        return result;
      }
      ...
      span.setAttribute('commit_story.journal.summaries_count', result.generated.length + weeklyResult.generated.length + monthlyResult.generated.length);
      return {
...
export async function triggerAutoWeeklySummaries(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.commands.trigger_auto_weekly_summaries', async (span) => {
    try {
      const { onProgress } = options;

      const unsummarizedWeeks = await findUnsummarizedWeeks(basePath);
      span.setAttribute('commit_story.journal.unsummarized_weeks_count', unsummarizedWeeks.length);
...
export async function triggerAutoMonthlySummaries(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.commands.trigger_auto_monthly_summaries', async (span) => {
    try {
      const { onProgress } = options;

      const unsummarizedMonths = await findUnsummarizedMonths(basePath);
      span.setAttribute('commit_story.journal.unsummarized_months_count', unsummarizedMonths.length);
