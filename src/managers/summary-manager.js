...
export async function readDayEntries(date, basePath = '.') {
  return tracer.startActiveSpan('commit_story.journal.read_day_entries', async (span) => {
...
export async function saveDailySummary(content, date, basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.journal.save_daily_summary', async (span) => {
...
export async function generateAndSaveDailySummary(date, basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.journal.generate_and_save_daily_summary', async (span) => {
...
export async function readWeekDailySummaries(weekStr, basePath = '.') {
  return tracer.startActiveSpan('commit_story.journal.read_week_daily_summaries', async (span) => {
    ...
    try {
      ...
      span.setAttribute('commit_story.journal.daily_summaries_count', summaries.length);
      return summaries;
    } catch (error) {
      span.recordException(error);
      span.setStatus({ code: SpanStatusCode.ERROR });
...
export async function saveWeeklySummary(content, weekStr, basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.journal.save_weekly_summary', async (span) => {
    span.setAttribute('commit_story.journal.week_label', weekStr);
    try {
...
export async function generateAndSaveWeeklySummary(weekStr, basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.journal.generate_and_save_weekly_summary', async (span) => {
    span.setAttribute('commit_story.journal.week_label', weekStr);
    try {
      ...
      span.setAttribute('commit_story.journal.daily_summaries_count', dailySummaries.length);
...
export async function readMonthWeeklySummaries(monthStr, basePath = '.') {
  return tracer.startActiveSpan('commit_story.journal.read_month_weekly_summaries', async (span) => {
    ...
    try {
      ...
      span.setAttribute('commit_story.journal.weekly_summaries_count', summaries.length);
      return summaries;
    } catch (error) {
      span.recordException(error);
      span.setStatus({ code: SpanStatusCode.ERROR });
...
export async function saveMonthlySummary(content, monthStr, basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.journal.save_monthly_summary', async (span) => {
    span.setAttribute('commit_story.journal.month_label', monthStr);
    try {
...
export async function generateAndSaveMonthlySummary(monthStr, basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.journal.generate_and_save_monthly_summary', async (span) => {
    span.setAttribute('commit_story.journal.month_label', monthStr);
    try {
      // Check for existing summary first
      if (!options.force) {
      ...
      span.setAttribute('commit_story.journal.weekly_summaries_count', weeklySummaries.length);
