...
export async function runSummarize(options) {
  return tracer.startActiveSpan('commit_story.summary.run_summarize', async (span) => {
    try {
      const { dates, force, basePath = '.', onProgress } = options;

      span.setAttribute('commit_story.summary.dates_count', String(dates.length));
      ...
      span.setAttribute('commit_story.summary.generated_count', String(result.generated.length));
      span.setAttribute('commit_story.summary.failed_count', String(result.failed.length));
      return result;
    } catch (error) {
...
export async function runWeeklySummarize(options) {
  return tracer.startActiveSpan('commit_story.summary.run_weekly_summarize', async (span) => {
    try {
      const { weeks, force, basePath = '.', onProgress } = options;

      span.setAttribute('commit_story.summary.weeks_count', String(weeks.length));
      span.setAttribute('commit_story.summary.force', force);
...
export async function runMonthlySummarize(options) {
  return tracer.startActiveSpan('commit_story.summary.run_monthly_summarize', async (span) => {
    try {
      const { months, force, basePath = '.', onProgress } = options;

      span.setAttribute('commit_story.summary.months_count', String(months.length));
      span.setAttribute('commit_story.summary.force', force);
