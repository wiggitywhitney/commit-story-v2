...
export async function dailySummaryNode(state) {
  return tracer.startActiveSpan('commit_story.ai.generate_daily_summary', async (span) => {
    try {
      const { entries, date } = state;
      span.setAttribute('commit_story.journal.entry_date', date);
      span.setAttribute('commit_story.summary.entry_count', entries?.length ?? 0);
...
export async function generateDailySummary(entries, date) {
  return tracer.startActiveSpan('commit_story.ai.run_daily_summary_graph', async (span) => {
    try {
      span.setAttribute('commit_story.journal.entry_date', date);
      span.setAttribute('commit_story.summary.entry_count', entries?.length ?? 0);
      ...
      span.setAttribute('commit_story.journal.errors_count', result.errors?.length ?? 0);
...
export async function weeklySummaryNode(state) {
  return tracer.startActiveSpan('commit_story.ai.generate_weekly_summary', async (span) => {
    try {
      const { dailySummaries, weekLabel } = state;
      span.setAttribute('commit_story.summary.week_label', weekLabel);
      span.setAttribute('commit_story.summary.entry_count', dailySummaries?.length ?? 0);
...
export async function generateWeeklySummary(dailySummaries, weekLabel) {
  return tracer.startActiveSpan('commit_story.ai.run_weekly_summary_graph', async (span) => {
    try {
      span.setAttribute('commit_story.summary.week_label', weekLabel);
      span.setAttribute('commit_story.summary.entry_count', dailySummaries?.length ?? 0);
...
export async function monthlySummaryNode(state) {
  return tracer.startActiveSpan('commit_story.ai.generate_monthly_summary', async (span) => {
    try {
      const { weeklySummaries, monthLabel } = state;
      span.setAttribute('commit_story.summary.month_label', monthLabel);
      span.setAttribute('commit_story.summary.entry_count', weeklySummaries?.length ?? 0);
...
export async function generateMonthlySummary(weeklySummaries, monthLabel) {
  return tracer.startActiveSpan('commit_story.ai.run_monthly_summary_graph', async (span) => {
    try {
      span.setAttribute('commit_story.summary.month_label', monthLabel);
      span.setAttribute('commit_story.summary.entry_count', weeklySummaries?.length ?? 0);
