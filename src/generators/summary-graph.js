...
export async function dailySummaryNode(state) {
  return tracer.startActiveSpan('commit_story.journal.daily_summary_node', async (span) => {
    span.setAttribute('commit_story.journal.entry_date', state.date ?? '');
    if (state.entries != null) {
      span.setAttribute('commit_story.journal.entries_count', state.entries.length);
    }
    try {
...
export async function generateDailySummary(entries, date) {
  return tracer.startActiveSpan('commit_story.journal.generate_daily_summary', async (span) => {
...
export async function weeklySummaryNode(state) {
  return tracer.startActiveSpan('commit_story.journal.weekly_summary_node', async (span) => {
    span.setAttribute('commit_story.journal.week_label', state.weekLabel ?? '');
    if (state.dailySummaries != null) {
      span.setAttribute('commit_story.journal.summaries_count', state.dailySummaries.length);
    }
    try {
...
export async function generateWeeklySummary(dailySummaries, weekLabel) {
  return tracer.startActiveSpan('commit_story.journal.generate_weekly_summary', async (span) => {
    span.setAttribute('commit_story.journal.week_label', weekLabel ?? '');
    if (dailySummaries != null) {
      span.setAttribute('commit_story.journal.summaries_count', dailySummaries.length);
    }
    try {
...
export async function monthlySummaryNode(state) {
  return tracer.startActiveSpan('commit_story.journal.monthly_summary_node', async (span) => {
    span.setAttribute('commit_story.journal.month_label', state.monthLabel ?? '');
    if (state.weeklySummaries != null) {
      span.setAttribute('commit_story.journal.summaries_count', state.weeklySummaries.length);
    }
    try {
...
export async function generateMonthlySummary(weeklySummaries, monthLabel) {
  return tracer.startActiveSpan('commit_story.journal.generate_monthly_summary', async (span) => {
    span.setAttribute('commit_story.journal.month_label', monthLabel ?? '');
    if (weeklySummaries != null) {
      span.setAttribute('commit_story.journal.summaries_count', weeklySummaries.length);
    }
    try {
