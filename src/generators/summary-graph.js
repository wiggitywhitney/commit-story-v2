...
export async function dailySummaryNode(state) {
  return tracer.startActiveSpan('commit_story.journal.daily_summary_node', async (span) => {
    ...
    span.setAttribute('commit_story.journal.entries_count', entries?.length ?? 0);
    try {
...
export async function weeklySummaryNode(state) {
  return tracer.startActiveSpan('commit_story.journal.weekly_summary_node', async (span) => {
    ...
    span.setAttribute('commit_story.journal.daily_summaries_count', dailySummaries?.length ?? 0);
    try {
...
export async function monthlySummaryNode(state) {
  return tracer.startActiveSpan('commit_story.journal.monthly_summary_node', async (span) => {
    ...
    span.setAttribute('commit_story.journal.weekly_summaries_count', weeklySummaries?.length ?? 0);
    try {
