...
async function runGit(args, { commitRef } = {}) {
  return tracer.startActiveSpan('commit_story.git_collector.run_git', async (span) => {
    if (commitRef != null) {
...
async function getCommitMetadata(commitRef = 'HEAD') {
  return tracer.startActiveSpan('commit_story.git_collector.get_commit_metadata', async (span) => {
    try {
      span.setAttribute('vcs.ref.head.revision', commitRef);
...
async function getCommitDiff(commitRef = 'HEAD') {
  return tracer.startActiveSpan('commit_story.git_collector.get_commit_diff', async (span) => {
    try {
      span.setAttribute('vcs.ref.head.revision', commitRef);
...
async function getMergeInfo(commitRef = 'HEAD') {
  return tracer.startActiveSpan('commit_story.git_collector.get_merge_info', async (span) => {
    try {
      span.setAttribute('vcs.ref.head.revision', commitRef);
      ...
      span.setAttribute('commit_story.git_collector.is_merge', parentCount > 1);
...
export async function getPreviousCommitTime(commitRef = 'HEAD') {
  return tracer.startActiveSpan('commit_story.git_collector.get_previous_commit_time', async (span) => {
    try {
      span.setAttribute('vcs.ref.head.revision', commitRef);
...
export async function getCommitData(commitRef = 'HEAD') {
  return tracer.startActiveSpan('commit_story.git_collector.get_commit_data', async (span) => {
    try {
      span.setAttribute('vcs.ref.head.revision', commitRef);
      ...
      if (mergeInfo != null) {
        span.setAttribute('commit_story.git_collector.is_merge', mergeInfo.isMerge);
      }
