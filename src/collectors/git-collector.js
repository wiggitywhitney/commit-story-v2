...
async function runGit(args, { commitRef } = {}) {
  return tracer.startActiveSpan('commit_story.git.run_command', async (span) => {
    span.setAttribute('commit_story.git.command', args[0]);
    try {
...
async function getCommitMetadata(commitRef = 'HEAD') {
  return tracer.startActiveSpan('commit_story.git.get_commit_metadata', async (span) => {
...
async function getCommitDiff(commitRef = 'HEAD') {
  return tracer.startActiveSpan('commit_story.git.get_commit_diff', async (span) => {
    span.setAttribute('vcs.ref.head.revision', commitRef);
    try {
      ...
      span.setAttribute('commit_story.git.diff_size', String(diff.length));
      return diff;
    } catch (error) {
      span.recordException(error);
      span.setStatus({ code: SpanStatusCode.ERROR });
...
async function getMergeInfo(commitRef = 'HEAD') {
  return tracer.startActiveSpan('commit_story.git.get_merge_info', async (span) => {
    span.setAttribute('vcs.ref.head.revision', commitRef);
    try {
      ...
      span.setAttribute('commit_story.git.is_merge', String(parentCount > 1));
  ...
  return tracer.startActiveSpan('commit_story.git.get_previous_commit_time', async (span) => {
    ...
    try {
      ...
      if (timestamps.length < 2) {
        span.setAttribute('commit_story.git.has_previous_commit', false);
        return null; // First commit, no previous
      }

      span.setAttribute('commit_story.git.has_previous_commit', true);
...
export async function getCommitData(commitRef = 'HEAD') {
  return tracer.startActiveSpan('commit_story.git.get_commit_data', async (span) => {
    ...
    try {
      ...
      span.setAttribute('commit_story.commit.files_changed', mergeInfo.parentCount);
      span.setAttribute('commit_story.git.is_merge', mergeInfo.isMerge);
      span.setAttribute('commit_story.git.diff_size', String(diff.length));
