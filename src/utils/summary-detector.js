...
export async function getDaysWithEntries(basePath = '.') {
  return tracer.startActiveSpan('commit_story.summary_detector.get_days_with_entries', async (span) => {
    try {
      ...
      dates.sort();
      span.setAttribute('commit_story.summary_detector.result_count', dates.length);
      return dates;
    } catch (error) {
...
export async function findUnsummarizedDays(basePath = '.', options = {}) {
  return tracer.startActiveSpan('commit_story.summary_detector.find_unsummarized_days', async (span) => {
    try {
      ...
      span.setAttribute('commit_story.summary_detector.result_count', result.length);
      return result;
    } catch (error) {
      span.recordException(error);
      span.setStatus({ code: SpanStatusCode.ERROR });
...
export async function getDaysWithDailySummaries(basePath = '.') {
  return tracer.startActiveSpan('commit_story.summary_detector.get_days_with_daily_summaries', async (span) => {
    try {
      const summariesDir = join(basePath, 'journal', 'summaries', 'daily');
      const datePattern = /^\d{4}-\d{2}-\d{2}\.md$/;
...
export async function findUnsummarizedWeeks(basePath = '.') {
  return tracer.startActiveSpan('commit_story.summary_detector.find_unsummarized_weeks', async (span) => {
    try {
      ...
      unsummarized.sort();
      span.setAttribute('commit_story.summary_detector.result_count', unsummarized.length);
      return unsummarized;
    } catch (error) {
      span.recordException(error);
      span.setStatus({ code: SpanStatusCode.ERROR });
...
export async function findUnsummarizedMonths(basePath = '.') {
  return tracer.startActiveSpan('commit_story.summary_detector.find_unsummarized_months', async (span) => {
    try {
