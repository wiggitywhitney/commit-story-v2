...
export async function handleSummarize(args) {
  return tracer.startActiveSpan('commit_story.summary.handle_summarize', async (span) => {
    try {
      const parsed = parseSummarizeArgs(args);

      span.setAttribute('commit_story.summary.force', Boolean(parsed.force));
      ...
      // Weekly mode
      if (parsed.weekly) {
        span.setAttribute('commit_story.summary.weeks_count', parsed.weeks.length);
      ...
      // Monthly mode
      if (parsed.monthly) {
        span.setAttribute('commit_story.summary.months_count', parsed.months.length);
      ...
      // Daily mode
      span.setAttribute('commit_story.summary.dates_count', parsed.dates.length);
