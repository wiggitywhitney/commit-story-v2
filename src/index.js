...
export async function handleSummarize(args) {
  return tracer.startActiveSpan('commit_story.journal.handle_summarize', async (span) => {
    try {
      const parsed = parseSummarizeArgs(args);
      span.setAttribute('commit_story.journal.force', !!parsed.force);
      ...
      if (parsed.weekly) {
        ...
        span.setAttribute('commit_story.journal.weeks_count', parsed.weeks.length);
        span.setAttribute('commit_story.journal.generated_count', result.generated.length);
        span.setAttribute('commit_story.journal.failed_count', result.failed.length);
      ...
      if (parsed.monthly) {
        ...
        span.setAttribute('commit_story.journal.months_count', parsed.months.length);
        span.setAttribute('commit_story.journal.generated_count', result.generated.length);
        span.setAttribute('commit_story.journal.failed_count', result.failed.length);
        return result.failed.length > 0 ? EXIT_ERROR : EXIT_SUCCESS;
      }
      ...
      span.setAttribute('commit_story.journal.dates_count', parsed.dates.length);
      span.setAttribute('commit_story.journal.generated_count', result.generated.length);
      span.setAttribute('commit_story.journal.failed_count', result.failed.length);
      return result.failed.length > 0 ? EXIT_ERROR : EXIT_SUCCESS;
    } catch (error) {
