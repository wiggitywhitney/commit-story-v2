...
export async function handleSummarize(args) {
  return tracer.startActiveSpan('commit_story.commands.handle_summarize', async (span) => {
    try {
      const parsed = parseSummarizeArgs(args);

      span.setAttribute('commit_story.summarize.force', !!parsed.force);
      ...
      span.setAttribute('commit_story.summarize.dates_count', parsed.dates.length);
      span.setAttribute('commit_story.summarize.failed_count', result.failed.length);
      return result.failed.length > 0 ? EXIT_ERROR : EXIT_SUCCESS;
    } catch (error) {
...
export async function main() {
  return tracer.startActiveSpan('commit_story.index.main', async (span) => {
    try {
      ...
      if (context.chat != null) {
        span.setAttribute('commit_story.context.messages_count', context.chat.messageCount);
      }
