...
- [~] **M4: Verify end-to-end observability triangle** *(deferred to issue [#79](https://github.com/wiggitywhitney/commit-story-v2/issues/79) — runs post-merge; see Decision 5)*
...
| 2026-06-18 | M4 deferred to standalone issue #79 (post-merge) | M4 requires an instrument branch generated from a main that has pino. All existing instrument branches (newest: June 10, 2026) predate M1/M2 and do not include src/logger.js. Additionally, otelcol-contrib binary is not installed on the dev machine. Deferring M4 to issue #79 unblocks the PRD from proceeding to /prd-done without losing the verification requirement. |
